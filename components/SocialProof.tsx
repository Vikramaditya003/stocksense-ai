...
export default function SocialProof() {
  return (
    <section className="bg-[#f6faf6] border-y border-[#bbcbba]/40">
      <div className="max-w-[1100px] mx-auto px-4 sm:px-6 py-5">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-[11px] font-semibold text-[#8a9a8a] uppercase tracking-widest whitespace-nowrap">
            ...
            {stats.map((s, i) => (
              <div
                ...
                className={`flex-1 sm:flex-none text-center px-6 sm:px-8 ${
                  i < stats.length - 1 ? "border-r border-[#bbcbba]/40" : ""
                }`}
              >
                <p className="text-[22px] font-bold text-[#006d34] tracking-tight tabular-nums">{s.value}</p>
                <p className="text-[11px] text-[#8a9a8a] mt-0.5">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
